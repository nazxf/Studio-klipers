...
Do not implement auth, database, Cloudflare R2, or FFmpeg yet.
...
## Phase 4 — Cloudflare R2 Upload

Implement MP4 upload to Cloudflare R2 with file type validation, file size validation, upload progress, video metadata storage, and redirect to video detail page after upload.
...
Create worker script that downloads source video from R2, runs FFmpeg, uploads output clip to R2, updates statuses, handles errors, and cleans temporary files.

## Phase 7 — Clip Preview and Download

Build clips page and clip detail page with preview player, metadata, download button, and processing/completed/failed states.
